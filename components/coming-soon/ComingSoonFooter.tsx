import Link from "next/link";
import { Instagram } from "lucide-react";
import { getMessages } from "@/lib/i18n";
import { CookieSettingsButton } from "@/components/consent/CookieSettingsButton";

const t = getMessages();

const footerLink =
  "rounded transition-colors hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand/60";

/* TikTok glyph from simple-icons (CC0) — lucide ships no TikTok icon. */
function TikTokIcon({ className }: { className?: string }) {
  return (
    <svg
      viewBox="0 0 24 24"
      fill="currentColor"
      aria-hidden="true"
      className={className}
    >
      <path d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.99-.32-2.15-.23-3.02.37-.63.41-1.11 1.04-1.36 1.75-.21.51-.15 1.07-.14 1.61.24 1.64 1.82 3.02 3.5 2.87 1.12-.01 2.19-.66 2.77-1.61.19-.33.4-.67.41-1.06.1-1.79.06-3.57.07-5.36.01-4.03-.01-8.05.02-12.07z" />
    </svg>
  );
}

/**
 * Rich footer for the coming-soon surface (landing + /danke + /confirm).
 * Carries the legal links, contact and the "Cookie-Einstellungen" entry point.
 */
export function ComingSoonFooter() {
  return (
    <footer className="relative border-t border-white/10">
      <div className="mx-auto flex w-full max-w-shell flex-col items-center justify-between gap-4 px-5 py-6 text-sm text-white/60 sm:flex-row sm:px-8">
        <nav
          aria-label="Rechtliches"
          className="flex flex-wrap items-center justify-center gap-x-5 gap-y-2 font-semibold"
        >
          <Link href="/impressum" className={footerLink}>
            {t.footer.impressum}
          </Link>
          <Link href="/datenschutz" className={footerLink}>
            {t.footer.datenschutz}
          </Link>
          <CookieSettingsButton className={`${footerLink} font-semibold`} />
        </nav>

        <a href={`mailto:${t.footer.contactEmail}`} className={footerLink}>
          {t.footer.contactEmail}
        </a>

        <div className="flex items-center gap-4">
          <a href="#" aria-label={t.footer.instagramLabel} className={footerLink}>
            <Instagram aria-hidden="true" className="h-5 w-5" />
          </a>
          <a href="#" aria-label={t.footer.tiktokLabel} className={footerLink}>
            <TikTokIcon className="h-5 w-5" />
          </a>
        </div>
      </div>
    </footer>
  );
}
